...
export default async function handler(req, res) {
  ...
  try {
    // Try specific query first, fall back to first word only
    let imageUrl = null;
    ...
    for (const query of queries) {
      ...
      if (unsplashData.urls?.regular) {
        ...
        console.log('Unsplash image found with query:', query);
        break;
      }
    }

    if (!imageUrl) throw new Error('Could not find any Unsplash image');
    ...
    // Upload to S3 via multipart form
    ...
    res.status(200).json({ success: true, assetId: meta.id, hostedUrl: meta.hostedUrl });
  } catch (err) {
